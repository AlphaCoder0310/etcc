...
def _compute():
    """Full scan, in a child process (default) or in-process."""
    if not ISOLATE:
        return scan(_ARGS["demo"], _ARGS["price"])
    import subprocess
    cmd = [sys.executable, os.path.abspath(__file__), "--json"]
    if _ARGS["demo"]:
        cmd += ["--demo", _ARGS["demo"]]
    if _ARGS["price"]:
        cmd += ["--price", _ARGS["price"]]
    r = subprocess.run(cmd, capture_output=True, text=True,
                       timeout=int(os.environ.get(
                           "DSCAN_TIMEOUT_S", 240)),
                       encoding="utf-8", errors="replace")
    if r.returncode != 0:
        raise RuntimeError("scan child failed: " +
                           (r.stderr or "")[-600:])
    out = r.stdout
    k = out.find("{")
    if k < 0:
        raise RuntimeError("scan child produced no JSON: " +
                           (r.stderr or "")[-400:])
    return json.loads(out[k:])
