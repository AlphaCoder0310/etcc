...
#rfq_mon .rm-meta{color:var(--faint);font-size:10px;
  margin-bottom:8px}
.rm-it{display:grid;
  grid-template-columns:48px minmax(96px,132px) 1fr;
  gap:0 8px;align-items:center;padding:2px 6px;border:0;
...
<header>LAGRANGE <small>CB Runs desk console &middot; build 2026-08-19.r72 &middot; one port (59988)</small>
...
function showTab(t){
  window.curTab=t;
  ['recon','delta','bau','blotter','rfq','twcb','nuke'].forEach(x=>{
...
window.addEventListener('unhandledrejection',ev=>{try{setS('rf_status','ERROR: '+(ev.reason&&ev.reason.message||ev.reason)+' \u00b7 '+(window._rfqBuild||'old-build'),'err');}catch(_){}});
    ...
    rfqTimer=setInterval(()=>{
      const ae=document.activeElement;
      if(ae && ae.dataset && (ae.dataset.rf||ae.dataset.rs)) return;
      if(window.curTab==='rfq') rfqLoad(true);
    },1000);
...
function rfqMonRender(){
  ...
  const items=rfqMonItems();
  $('rf_mon_n').textContent=items.length;
  el.innerHTML=items.map(it=>
    `<div class="rm-it" data-k="${it.k}" data-id="${it.id}" title="score ${Math.round(it.score)}">`+
...
if __name__ == "__main__":
    ...
    threading.Thread(target=_rfq_engine_loop,
                     daemon=True).start()
    print("=" * 62)
    print("  LAGRANGE  BUILD r72  ·  %s" % os.path.abspath(__file__))
